...
@user_passes_test(
    is_volunteer, redirect_field_name=None, login_url=reverse_lazy("home:dashboard")
)
...
def ajax_volunteers_list(request):
    ...
    for vol_sch in vol_to_show:
        ...
        data[key] = [
            vol_sch.volun.roll_no,
            vol_sch.volun.profile.get_full_name,
    ...
    if request.method == "POST":
        ...
        if volun.roll_no != roll_no:
            ...
            if duplicate_roll_check.exists():
                context[
                    "update_error"
                ] = "A volunteer with entered roll no. already exists."
...
def update_schedule(request):
    ...
    context = {
        "day": Schedule.DAY,
        "update_req": UpdateScheduleRequest.objects.filter(volun=volun).order_by(
            "-date"
        ),
