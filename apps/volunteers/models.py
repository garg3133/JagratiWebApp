...
class Designation(models.Model):
    desig_id = models.CharField(max_length=10,
                                verbose_name='Designation ID',
                                unique=True)
    name = models.CharField(max_length=100)
    parent_desig = models.ForeignKey('self',
...
class Volunteer(models.Model):
    ...
    roll_no = models.CharField(verbose_name="Roll Number",
                               max_length=8,
                               unique=True)
    ...
    desig = models.ForeignKey(Designation,
                              on_delete=models.SET_NULL,
                              null=True,
                              blank=True)  # null --> Normal Volunteer
...
class VolunteerSchedule(models.Model):
    volun = models.ForeignKey(Volunteer,
                              on_delete=models.CASCADE,
                              related_name='volun_schedules')
...
class UpdateScheduleRequest(models.Model):
    volun = models.ForeignKey(Volunteer,
                              on_delete=models.CASCADE,
                              related_name='update_sch_requests')
    previous_schedule = models.ForeignKey(
        ...
        null=True,
        blank=True)
    new_schedule = models.ForeignKey(Schedule,
                                     on_delete=models.CASCADE,
                                     related_name='update_sch_requests_to')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.approved is True or self.by_admin is True:  # Schedule is updated in only these two cases
            ...
            if prev_sch.exists():
                ...
                vol_sch = VolunteerSchedule(volun=self.volun,
                                            schedule=self.updated_schedule)
                vol_sch.save()
        # For CPanel
