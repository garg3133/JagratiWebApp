...
@login_required
@user_passes_test(
    has_authenticated_profile, login_url=reverse_lazy(
...
def attendance(request):
    ...
    if today_cal.class_scheduled:
        if not StudentAttendance.objects.filter(cal_date__date=today_date).exists():
            ...
            StudentAttendance.objects.filter(cal_date__date=today_date).count()
    ...
    context["stu_att_today"] = StudentAttendance.objects.filter(
        cal_date=today_cal, student__school_class__range=(1, 3)
...
def ajax_attendance(request):
    ...
    for stu_att in stu_att_today:
        key = (
            str(stu_att.student.school_class) + stu_att.student.get_full_name
        )  # For sorting purpose.
        data[key] = [
            stu_att.student.id,
            stu_att.student.get_full_name,
            stu_att.student.school_class,
            stu_att.present,
        ]
        ...
        for i in range(3, max_row + 1):
            ...
            if not Student.objects.filter(
                ...
                for day, day_name in Schedule.DAY:
                    print(day)
                    stu_sch = StudentSchedule(
                        student=student,
                        schedule=Schedule.objects.get(
                            day=day, section__section_id="4A"
                        ),
                    )
                    stu_sch.save()
        # Delete the file
