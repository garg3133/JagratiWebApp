    ...
    def post(self, request):
        ...
        if user is not None:
            ...
            if not user.is_active:
                ...
                data[
                    "error_message"
                ] = "User Account not Activated. Please check your inbox/spam for an Account Activation Email."
            ...
            device_info = AuthorisedDevice.objects.filter(
...
def complete_profile_view(request):
    ...
    profile = Profile.objects.filter(user=user)
    if profile.exists():
        data = {"response": "Error", "error_message": "Profile already exists."}
        return Response(data, status=400)
