    ...
    if request.method == "POST":
        if request.POST.get("submit") == "login":
            ...
            if user is not None:
                ...
                if not user.is_active:
                    context[
                        "login_error"
                    ] = 'Account not Activated.<br><a href="#">Resend Activation Email?</a>'
                ...
                if not user.auth and profile.exists():
                    ...
                    context[
                        "login_error"
                    ] = "User is not yet authenticated by the Admin. Kindly contact Admin."
                    return render(request, "accounts/login_signup.html", context)
                ...
                if (
                    user.exists()
                    and user[0].check_password(password)
                    and not user[0].is_active
                ):
        ...
        if request.POST.get("submit") == "sign_up":
            ...
            if user.exists():
                ...
                return render(
                    request, "accounts/login_signup.html", {"signup_error": error}
                )
...
def account_activation(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        return render(
            request, "accounts/token_expired.html", {"msg": msg, "act_token": True}
        )
...
def account_authentication(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        html_message = render_to_string(
            "accounts/email/account_authenticated_email.html",
            {
