...
@login_required
@user_passes_test(
    has_authenticated_profile, login_url=reverse_lazy("accounts:complete_profile")
)
def dashboard(request):
    ...
    if query_date_str and query_section:
        ...
        cw_hw = ClassworkHomework.objects.filter(
            cal_date=calendar, section__section_id=query_section
        ).first()
        context["cw_hw"] = cw_hw

        # Students Attendance
        student_attendance = StudentAttendance.objects.filter(
            cal_date=calendar, present=True
        ).order_by("student__school_class")
